...
HiInclusiveJetAnalyzer::HiInclusiveJetAnalyzer(const edm::ParameterSet& iConfig)
{
  ...
  doJetConstituents_ = iConfig.getUntrackedParameter<bool>("doJetConstituents", true);
  ...
  //reWTA reclustering
  doWTARecluster_ = iConfig.getUntrackedParameter<bool>("doWTARecluster", true);
...
HiInclusiveJetAnalyzer::beginJob() {
  ...
  std::cout << "made it to filling in the jet pt:" << jets_.jtpt << endl;
  ...
  if(doWTARecluster_){
    std::cout << "made it to reclustering:" << jets_.WTAeta << endl;
